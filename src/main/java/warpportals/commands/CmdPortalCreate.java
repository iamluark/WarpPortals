                    ...
                    if (main.iPortalManager.getPortalInfo(args[0].trim()) == null) {
                        try {
                            ...
                            if (args[1].matches("\\([0-9]+,[0-9]+,[0-9]+\\)")) {
                                ...
                                if (tpCoords == null)
                                    sender.sendMessage("Couldn't find the WarpPortal Destination \"" + args[1] + "\"");
                            }
                            if (tpCoords != null) {
                                /*
                                ...
                                sender.sendMessage("The 2nd param is the WarpPortal Destination. It must be in the format (x,y,z) or the name of a WarpPortal Destination set with /wp-destination-create [name]");
                        } catch (NullWorldException e) {
                            sender.sendMessage("The world specified in the destination is invalid. \"" + e.getWorldName() + "\" does not exist.");
                        }
                    } else
                        sender.sendMessage("\"" + args[0].trim() + "\" is already a Portal!");
                } else
                    sender.sendMessage("The first param is the Portal Name. You can only use a-z, A-Z, 0-9.");
            } catch (Exception e) {
                sender.sendMessage("Error creating Portal");
            }
        } else
            return false;
        return true;
